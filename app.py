...
def predict():
    ...
    if file and allowed_file(file.filename):
        ...
        response_data = {
            'success': True,
            'message': 'Prediksi Tinggi Badan Berhasil',
            'data': {
                'image_url': publicUrl,
        ...
        response_data = {
            'success': False,
            'message': 'Mohon input kembali foto anak, pastikan AruCo Marker Jelas!',
            'data': [],
        }
        return jsonify(response_data)
...
def not_found(error):
    response_data = {'message': 'Endpoint not found', 'status_code': 404}
    return jsonify(response_data)
